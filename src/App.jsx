    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-[280px] rounded-[2rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 relative border border-white/20">
        <div className="bg-slate-800 py-4 px-6 text-center text-white relative">
          <h3 className="text-base font-black tracking-tighter font-noto">센트럴처치 안내</h3>
          <button 
            onClick={onClose} 
        ...
        <div className="p-4 space-y-4 font-noto break-keep text-center">
          <div>
        ...
        <button onClick={onClose} className="w-full py-3 bg-slate-800 text-white font-black uppercase tracking-[0.2em] text-[9px] hover:bg-black transition-colors font-noto">확인했습니다</button>
      </div>
    </div>
  );
};
...
const Home = ({ onWorshipClick }) => (
  <main className="relative min-h-screen flex items-center bg-white overflow-hidden pt-[140px] md:pt-[200px] pb-20 font-noto">
...
const About = () => (
  <main className="pt-[140px] md:pt-[200px] pb-32 px-6 bg-white animate-in fade-in duration-1000 font-noto">
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-10 font-sans font-bold">
...
const Worship = () => (
  <section className="pt-[140px] md:pt-[200px] pb-32 px-6 bg-white min-h-screen font-noto">
...
const Sermon = ({ videos, isLoading, onVideoSelect }) => (
  <main className="pt-[140px] md:pt-[200px] pb-32 px-6 bg-white min-h-screen font-noto">
    <div className="max-w-7xl mx-auto">
       <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 md:mb-20 gap-8">
         ...
         <a href="https://youtube.com/@centralchurch5467?si=zOJh5JBLDqAlI1fF" target="_blank" rel="noopener noreferrer" className="bg-red-600 text-white px-8 md:px-10 py-4 rounded-full font-black text-[11px] tracking-widest hover:bg-red-700 transition-all uppercase flex items-center gap-3 shadow-xl shadow-red-50">
            ...
            {videos.length > 0 ? (
              videos.map((video, i) => (
                <div key={i} onClick={() => onVideoSelect(video.id.videoId)} className="group cursor-pointer block text-left">
                  ...
                  <h4 className="text-lg md:text-xl font-black text-gray-900 mb-2 group-hover:text-indigo-600 transition-colors line-clamp-2 leading-snug break-keep">
                    {video.snippet.title}
                  </h4>
          ...
          <div className="lg:col-span-5 space-y-10 md:space-y-12 text-left">
            ...
            <div className="space-y-10 md:space-y-12 break-keep">
              <div className="flex gap-5 md:gap-6 items-start group">
                ...
                <div><h4 className="font-black text-xl mb-3 text-gray-900 tracking-tight">지하철</h4><p className="text-sm md:text-base text-gray-500 leading-relaxed font-noto"><span className="text-indigo-600 font-black text-lg">사당역</span> (2, 4호선) 11~14번 출구 <span className="text-gray-400 font-bold ml-1">(도보 5분)</span><br /><span className="text-indigo-600 font-black text-lg">이수역</span> (4, 7호선) 4, 5번 출구 <span className="text-gray-400 font-bold ml-1">(도보 10분)</span></p></div>
              </div>
              <div className="flex gap-5 md:gap-6 items-start group">
                ...
                <div><h4 className="font-black text-xl mb-3 text-gray-900 tracking-tight">버스</h4><p className="text-sm md:text-base text-gray-500 leading-relaxed font-noto"><span className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-lg font-black mr-2">350, 4212, 4318, 동작09 등</span><br />방배노인종합복지관(중) 하차 <span className="text-gray-400 font-bold ml-1">(도보 3분)</span></p></div>
              </div>
              <div className="flex gap-5 md:gap-6 items-start group">
                ...
                <div><h4 className="font-black text-xl mb-3 text-gray-900 tracking-tight font-black">자가운전</h4><p className="text-sm md:text-base text-gray-500 leading-relaxed font-noto"><span className="text-indigo-600 font-black">'서초구 방배천로 40-2'</span><br /><span className="text-xs text-gray-400 font-black">교회 앞 공영주차장을 이용해 주시기 바랍니다.</span></p></div>
              </div>
            </div>
          </div>
          <div className="lg:col-span-7 h-[500px] md:h-full min-h-[500px]">
            <div className="bg-white rounded-[3rem] md:rounded-[5rem] overflow-hidden shadow-2xl border-[8px] md:border-[16px] border-white h-full relative shadow-indigo-100/50">
              ...
              <div className="absolute bottom-6 left-6 right-6 md:bottom-12 md:left-12 md:right-12 z-20 bg-white/90 backdrop-blur-xl p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-2xl flex items-center justify-between border border-white/20">
                <div className="font-sans text-left font-black"><p className="font-black text-gray-900 text-lg md:text-2xl tracking-tighter uppercase font-noto">센트럴처치</p><div className="text-[10px] md:text-[11px] text-gray-400 mt-1 font-sans flex flex-col gap-0.5 font-noto font-black"><span>서울특별시 서초구 방배천로 40-2</span><span className="opacity-70">서초구 방배2동 453-6 2층, 3층</span></div></div>
  ...
  useEffect(() => {
    if (YOUTUBE_API_KEY && YOUTUBE_CHANNEL_ID) {
      const fetchVideos = async () => {
        ...
        } catch (err) { console.error("유튜브 에러:", err); }
    ...
    <div className="min-h-screen bg-white font-noto text-gray-900 selection:bg-indigo-100 selection:text-indigo-900 tracking-tight overflow-x-hidden font-bold">
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-white z-[9999] flex flex-col animate-in fade-in slide-in-from-right duration-300 font-bold font-sans">
          ...
          <div className="mt-auto p-10 border-t border-gray-50 bg-gray-50/50 font-sans uppercase"><p className="text-[10px] font-black text-gray-300 tracking-[0.4em]">© 2026 Central Church</p></div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center font-sans">
          <div className="flex items-center cursor-pointer group font-bold" onClick={() => setActiveTab('home')}>
            <div className="relative h-20 md:h-28 flex items-center">
               ...
               <div className="hidden items-center gap-2 font-bold uppercase"><BookOpen size={24} className="text-indigo-600 font-bold" /><span className="text-xl font-black text-gray-900 tracking-tighter font-noto">Central Church</span></div>
            </div>
          </div>
          ...
          <div className="md:hidden flex items-center font-bold"><button onClick={() => setIsMenuOpen(true)} className="p-3 -mr-3 text-gray-900 active:bg-gray-50 rounded-full transition-colors relative z-[1100]"><Menu size={28} strokeWidth={1.5} /></button></div>
        </div>
      </nav>
